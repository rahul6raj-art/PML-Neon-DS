...
**Agent note:** Cursor **`.cursor/rules/pml-screen-patterns.mdc`** (`alwaysApply: true`) requires reading this file (with **`DESIGN.md`** and **`design-system-schema/layout-candidates.md`**) when implementing or planning screens, widgets, flows, or registry-aligned UI—the user does **not** need to attach it.
...
- **`SectionHeader` (PML app):** For new section titles, **always default to `size="extra-large"`** unless PRD/Figma explicitly specifies a smaller size for density, or the heading is **static** and must **not** show a chevron (today: use **`size="large"`** + **`showChevron={false}`** + **`trailing="none"`** only in that narrow case — **`extra-large`** always includes a chevron in the current component). For list / drill-in sections, use **`trailing="none"`** (patterns often mis-labeled “See more”, “View all”, “View more”, or “+ Add” beside the title). When a **visual reference or PRD** shows **“View all”**, **“View more”**, **“See all”**, or similar **as a link next to the section title**, implement **`extra-large`** + **`trailing="none"`** (chevron is implicit for **`extra-large`**) — do **not** recreate that copy with **`trailing="link"`** unless PRD/Figma explicitly requires the text control. **Do not** use **`trailing="link"`** (or trailing text as a faux link) for that affordance; the chevron is the standard treatment.
- **Static-only titles** (no navigation, expand, or fuller list): do **not** use a chevron that implies interaction. Because **`extra-large`** always shows a chevron, use the **`large`** + **`showChevron={false}`** exception above — do not default the whole screen to **`large`** headers to avoid this; keep **`extra-large`** everywhere else.
...
- **Card-like stacked lists** (single **`--surface-level-1`** stack with multiple rows — **`Card`** or equivalent in feature CSS): Use **`border-radius: var(--radius-24)`** (match **`Card`**). **Row dividers** follow **`.cursor/rules/pml-screen-patterns.mdc` → Tokens → Card-like stacked list surfaces:** either **(1)** **custom rows** — inset **`var(--card-padding-inline)`** each side via **`::before`** / margins and a **block wrapper** when rows are **`display: flex`**, or **(2)** **`Card` + stacked `ListItem`** with default **`Card`** **`padding-inline`** — hairlines **edge-to-edge across the padded card body**, scoped on the **`Card`** class (**`left`/`right: 0`** on **`li-item__separator`**, override **`--inset`** unless spec says otherwise). Examples and removed-demo notes: **`docs/LEARNINGS.md`**. **First row:** **`padding-top: var(--card-padding-block)`**; **last row:** **`padding-bottom: var(--card-padding-block)`**; **middle** rows: denser vertical pad (e.g. **`--spacing-12`**). **Horizontal** row padding **`var(--card-padding-inline)`** like **`Card`** / **`ListItem`**. **`SectionHeader` + stack:** use **either** section wrapper **`--card-padding-block`** vertical **or** first/last row interior pad — **not** both stacked to **48px** unless **PRD/Figma** requires it. **PRD/Figma** may override for a deliberate variant.
