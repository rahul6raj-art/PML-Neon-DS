...
**Shared design system library** — lives at repo root. Use when icons match the design system or as sensible fallbacks for AI-generated designs (see `.cursor/vertical-setup.md` §Icons).

SVG exports from the design system icon library, for use in React (e.g. inline SVGs or a small icon component with `currentColor`).

- **`svg/glyphs/`** — Single-color glyph icons (24×24). Use `currentColor` for fill in React.

## Two styles per glyph

Most glyphs have **two variants** (outline vs filled). Both are exported:
...
Source: Paytm Design System — Icons v3.

**Current export:** 201 glyph SVGs in `svg/glyphs/` (outline + filled where applicable, single file for single-style icons). Re-export any missing icons using the same pattern (COMPONENT_SET → export `children[0]` and `children[1]`; replace `fill="#282828"` with `fill="currentColor"`).

- **`svg/flat/`** — Multi-color illustrative icons (24×24). Do **not** use `currentColor`; keep original fills.  
  **Export:** 20 flat SVGs from Icons v3, page “Flat”. Naming from `.cursor/rules/pods3/icons/flat.mdc` (e.g. `trophy.svg`, `cashback.svg`, `offer_badge.svg`).
